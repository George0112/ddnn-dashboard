...
parser.add_argument("--n-cut-points", type=int, default=22)
...
rps = args.rps
period = args.period
n_cut_points = args.n_cut_points

print(model_name, start_from, devices, rps, period, n_cut_points)
exit()
...
part2_mem = []
cut_points = []
for i in range(n_cut_points):
    print("cut point: %d" %(i))
    futures = []
    fail = 0
    res = requests.put('http://dashboard.tesla.cs.nthu.edu.tw:32510/model', data={
        "cut_points": [i], "output_points": [-1], "model": model_name, "start_from": start_from, "devices": devices
        })
    print(res.text)
    start = time.time()
