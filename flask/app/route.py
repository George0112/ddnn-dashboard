...
def model():
  ...
  if request.method=='POST':
    ...
    cmd = "python3 app/generate_yaml.py --model " + model + " --cut-point " + " ".join([str(e) for e in cut_points])
...
def info():
  ...
  for n in names:
    ...
    result.append({"name": n, "info": info, "cuttable": cuttable, "endPoint": 'http://tesla.cs.nthu.edu.tw:32510/%s' % (n)})
